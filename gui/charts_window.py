...
from gui.gui_utils import COLORS, FONTS, create_header_frame, create_button, setup_dark_theme
...
class ChartsWindow:
    ...
    def __init__(self, parent, user_id, db, accounts):
        ...
        if not accounts:
            messagebox.showinfo("No Data", "Please create an account first to view charts.")
            return
    ...
    def create_widgets(self):
        ...
        # Header
        create_header_frame(self.window, "Analytics Dashboard", "Financial Insights & Trends", height=100)
    ...
    def show_spending_pie_chart(self):
        ...
        if not category_totals:
            self.show_no_data_message("No spending data available yet.\nMake some transactions to see your spending breakdown.")
            return
        ...
        colors_list = ['#00ff88', '#00d4ff', '#a855f7', '#ff6b35', '#ffd93d',
                      '#ff4757', '#5f27cd', '#00d2d3', '#ff9ff3', '#48dbfb']
        ...
        ax.set_title('Spending by Category', fontsize=16, fontweight='bold',
                    pad=20, color=COLORS['text_bright'])
        ax.axis('equal')

        # Add total spending label
        total_spending = sum(amounts)
        fig.text(0.5, 0.02, f'Total Spending: ${total_spending:.2f}',
    ...
    def show_balance_history(self):
        ...
        ax.plot(dates, balances, color=COLORS['accent'], linewidth=2, marker='o', markersize=4)
        ax.fill_between(dates, balances, alpha=0.3, color=COLORS['accent'])

        ax.set_xlabel('Date', fontsize=12, fontweight='bold')
        ax.set_ylabel('Balance ($)', fontsize=12, fontweight='bold')
        ax.set_title(f'Balance History - {account.get_account_type()} ({account.account_number[-4:]})',
        ...
        if balances:
            ...
            ax.axhline(y=current_balance, color='red', linestyle='--', alpha=0.5, linewidth=1)
            ax.text(0.02, 0.98, f'Current: ${current_balance:.2f}',
    ...
    def show_monthly_comparison(self):
        ...
        for account in self.accounts:
            ...
            for trans in transactions:
                try:
                    ...
                    if trans['transaction_type'] in ['Deposit', 'Transfer In', 'Payment']:
                        monthly_data[month_key]['income'] += trans['amount']
                    elif trans['transaction_type'] in ['Withdrawal', 'Transfer Out', 'Credit Purchase']:
                        monthly_data[month_key]['expenses'] += trans['amount']
                except:
                    continue

        if not monthly_data:
            self.show_no_data_message("No transaction data available for monthly comparison.")
            return
        ...
        bars1 = ax.bar([i - width/2 for i in x], income, width, label='Income',
                      color=COLORS['accent'], alpha=0.8)
        bars2 = ax.bar([i + width/2 for i in x], expenses, width, label='Expenses',
    ...
    def show_accounts_overview(self):
        ...
        fig.text(0.5, 0.03, f'Total Balance Across All Accounts: ${total_balance:.2f}',
                ha='center', fontsize=12, fontweight='bold')
